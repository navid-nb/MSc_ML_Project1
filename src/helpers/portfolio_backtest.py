...
def calculate_portfolio_returns(
    ...
    portfolio_returns = (
        df.groupby(level=date_col)
        .apply(lambda x: (x[weights_col] * x[returns_col]).sum())
    )
...
def calculate_performance_metrics(
    ...
    sortino = (ann_return - rf_rate) / (downside_std * np.sqrt(periods_per_year)) if downside_std > 0 else 0.0
