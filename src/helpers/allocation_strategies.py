...
def strategy_a6_max_sharpe(
    ...
    try:
        ...
        if n_stocks > 1:
            ...
            cov_matrix = returns_df.loc[agreed_indices].unstack(level="permno").fillna(0).cov().values
            ...
            # Optimize
            result = minimize(neg_sharpe, w0, method='SLSQP', bounds=bounds, constraints=constraints, options={'maxiter': 100})
            
            if result.success:
                weights[agreed_indices] = result.x
            else:
                # Fallback to equal weight
                return strategy_a1_equal_weighted(scores, long_mask, short_mask, long_target, short_target, max_position_size)
        else:
            # Fallback to equal weight
            return strategy_a1_equal_weighted(scores, long_mask, short_mask, long_target, short_target, max_position_size)
    except:
