...
DEF_PY_CANDIDATES = [
    ...
    "python13", "python12", "python11", "python10",
    "python3", "python",
...
def build_for_interpreter(py_exe: str, lock_path: Path) -> None:
    ...
    try:
        sh([py_exe, "-m", "pip", "download",
            "--only-binary", ":all:",
            "--exists-action", "i",      # don't overwrite existing artifacts
            "-r", str(lock_path),
            "-d", str(WHEELS)])
    ...
    sh([py_exe, "-m", "pip", "wheel",
        "--exists-action", "i",          # keep existing wheels intact
        "-r", str(lock_path),
        "-w", str(WHEELS)])
...
def verify_coverage(lock_path: Path) -> None:
    ...
    pins = [norm_name(ln) for ln in lock_path.read_text().splitlines()
            if ln and not ln.startswith("#") and not ln.startswith("--")]
    wheel_names = {wf.name.split("-")[0].lower().replace("_", "-")
    ...
    if missing:
        ...
        for m in missing:
            print(" -", m)
        print("They may still be satisfied by transitive deps or env markers, but review is recommended.")
    else:
...
def main() -> None:
    ...
    ap.add_argument("--py", nargs="*", default=None,
                    help="Interpreter commands/paths to build for (default: try 3.13→3.10)")
    ap.add_argument("--lock", default=str(BASE_LOCK),
                    help="Base lock file to use (default: requirements.lock)")
    ap.add_argument("--write-per-version-locks", action="store_true",
    ...
    REQ_TXT.write_text("--no-index\n--find-links ./wheels\n-r requirements.lock\n", encoding="utf-8")
    print(f"\n[done] Unified wheelhouse in ./wheels for {len(chosen)} interpreters.")
    print(f"       requirements.txt written; per-version locks present where requested.")
