def run_full_pipeline(
    ...
    if tickers is None:
        ...
        assert set(tickers).issubset(set(available_tickers))
    ...
    if HYPERPARAMETER_TUNING:
        ...
        agg = cv_df.groupby("param_logistic__l1_ratio")["mean_error"].min().reindex(l1_ratios)

        plt.figure(figsize=(10, 6))
        plt.plot(list(agg.index), list(agg.values), marker="o", markersize=4)
        plt.axvline(
            l1_ratio_star, color="r", linestyle="--", label=f"l1_ratio* = {l1_ratio_star:.3f}"
        )
        plt.xlabel("L1 Ratio")
    ...
    if HYPERPARAMETER_TUNING_LINEAR:
        ...
        agg = cv_df.groupby("param_lasso__l1_ratio")["mean_rmse"].min().reindex(l1_ratios_lin)
        ...
        plt.axvline(
            ...
            color="r",
            linestyle="--",
            label=f"l1_ratio* = {l1_ratio_star_lin:.3f}",
        )
        plt.xlabel("L1 Ratio")
    ...
    df_signals["linear_signal_long"] = (
        df_signals["expected_return"] > EXPECTED_RETURN_THRESHOLD_LONG
    )
    df_signals["linear_signal_short"] = (
        df_signals["expected_return"] < -EXPECTED_RETURN_THRESHOLD_SHORT
    )
    ...
    for score_name, (score_col, score_desc) in SCORING_METHODS.items():
        for alloc_strategy in ALLOCATION_STRATEGIES:
            ...
            try:
                ...
                errors.append(
                    {"scoring": score_name, "allocation": alloc_strategy, "error": str(e)}
                )
                continue
    ...
    df_oos_signals = df_signals[
        df_signals.index.get_level_values("date").isin(dates_out_sample)
    ].copy()
    ...
    for idx_out, strategy_dict in enumerate(strategies_to_output):
        ...
        output_file_monthly = (
            f"{reports_dir}/oos_{strat_scoring}_{allocation_strategy_name}_monthly.html"
        )
