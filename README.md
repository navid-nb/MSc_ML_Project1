...
5. **Sequential data integration with quality assurance**
    ...
    - **IBES Consensus**: `pre_qa_ibes_statsumu(...)` -> `prepare_ibes_for_daily_merge(...)` ->
      `join_prices_with_ibes(...)` -> `post_join_qa_prices_with_ibes(...)`
    - **IBES Actuals**: `pre_qa_ibes_actu(...)` -> `prepare_ibes_actu_for_daily_merge(...)` ->
